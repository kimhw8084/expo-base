...
function StaticFieldExamples() {
  ...
  return <VStack gap="xl">
    ...
    <SelectField id="issuer" label="Issuer" value={issuer} onChange={setIssuer} options={[{ value: 'amex', label: 'American Express' }, { value: 'capital-one', label: 'Capital One' }, { value: 'chase', label: 'Chase' }, { value: 'citi', label: 'Citi' }]} description="This dropdown uses the shared collision-aware Overlay Manager." />
    <ComboboxField id="combobox" label="Primary issuer" value={issuer} onChange={setIssuer} options={[{ value: 'amex', label: 'American Express' }, { value: 'capital-one', label: 'Capital One' }, { value: 'chase', label: 'Chase' }, { value: 'citi', label: 'Citi' }]} description="Search, keyboard navigation, loading, empty, and retry state remain with one field owner." />
    <MultiSelectField id="networks" label="Accepted networks" values={networks} onChange={setNetworks} options={[{ value: 'visa', label: 'Visa' }, { value: 'mastercard', label: 'Mastercard' }, { value: 'amex', label: 'American Express' }]} />
    <TextArea id="notes" label="Notes" value={notes} onChangeText={setNotes} />
    <Checkbox label="Include annual fees" checked={includeFees} onChange={setIncludeFees} description="Checkbox rows use the same focus and press language." />
    <CheckboxGroup id="benefits" label="Benefit categories" values={benefits} onChange={setBenefits} options={[{ value: 'travel', label: 'Travel' }, { value: 'dining', label: 'Dining' }, { value: 'credits', label: 'Credits' }]} />
    <RadioGroup label="Sort cards" value={sortCards} onChange={setSortCards} options={[{ value: 'newest', label: 'Newest first' }, { value: 'value', label: 'Highest value', description: 'Uses estimated net value.' }]} />
    <SegmentedField id="density" label="Display density" value={density} onChange={setDensity} options={[{ value: 'comfortable', label: 'Comfortable' }, { value: 'compact', label: 'Compact' }]} />
    <SwitchField label="Bonus notifications" value={bonusNotifications} onChange={setBonusNotifications} description="Uses the native switch control inside system-owned spacing." />
    <NumberStepper id="priority" label="Review priority" value={priority} minimum={1} maximum={5} onChange={setPriority} description="A bounded adjustment stays touch-safe without a product-local button pair." />
    <CodeField id="verification-code" label="Verification code" value={code} onChangeText={setCode} description="Paste and sequential focus work through the shared provider-neutral code entry owner." />
