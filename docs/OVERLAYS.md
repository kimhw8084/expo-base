...
3. Escape/back closes safe overlays. Destructive alerts can disable backdrop and Escape dismissal and require explicit action. Dialogs and sheets trap keyboard focus while open, focus their first action/content target, and restore focus only when no newer overlay owns it. Trapping and restoration skip disconnected, disabled, hidden, inert, aria-disabled, aria-hidden, display-none, visibility-hidden, or fully transparent targets. An optional `restoreFocusFallbackId` on Dialog, BottomSheet, or Popover supplies the owned fallback only when the original target is ineligible; if both targets are ineligible, focus is left where it is. Trigger-unmount restoration is safely skipped.
