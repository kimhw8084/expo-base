...
| `settings-form` | Build account, profile, or settings forms with dirty protection and responsive actions. | `SettingsLayout`, `FormWorkspaceLayout`, `FormSectionGroup`, `useExpoBaseFormLifecycle` | optional | required | `preferences` | yes | [/forms](/forms) |
...
| `authentication` | Build provider-neutral sign-in, recovery, verification, or unlock flows. | `AuthenticationLayout`, `useExpoBaseAuth`, `ExpoBaseSessionSecurityBootstrap` | none | optional | `local-auth`, `secure-storage` | manual composition | [/sign-in](/sign-in) |
| `account-security` | Show account security state and local-unlock controls. | `ProfileLayout`, `useExpoBaseSessionSecurity`, `PermissionRationaleLayout` | optional | optional | `local-auth`, `secure-storage` | manual composition | [/session-security](/session-security) |
...
| `import-workflow` | Select a document or media item, present selected media safely, validate it, and hand it to a product upload mutation. | `ImportWorkflowLayout`, `useExpoBaseDocumentPicker`, `MediaFrame`, `useExpoBaseMutation` | optional | optional | `media` | yes | [/workflows](/workflows) |
...
| `permission-rationale` | Explain an optional capability before requesting it or opening settings. | `PermissionRationaleLayout`, `StateView`, `ExpoBaseCapabilityAvailability` | none | none | `media`, `notifications`, `local-auth` | yes | [/workflows](/workflows) |
| `offline-workspace` | Retain usable data while offline and offer explicit refresh after reconnect. | `OfflineWorkspaceLayout`, `ServerStateContent`, `ExpoBaseServerStateRuntimeBridge` | required | none | `runtime-signals` | yes | [/workflows](/workflows) |
