...
Use `useExpoBaseAsyncAction` only when the action has no query/cache effects. Remote reads and
writes that require identity, deduplication, stale data, invalidation, or optimistic bookkeeping
belong to `@expo-base/server-state`; see [`SERVER_STATE.md`](./SERVER_STATE.md). These owners
are complementary, not competing mutation systems.

`useExpoBaseAsyncAction` from `@expo-base/runtime` is the canonical hook for feature-owned async actions. It exposes `idle`, `loading`, `success`, and `error` state, shares the in-flight promise when an action is invoked again before completion, and ignores late completion after reset or unmount. It does not pretend to cancel an adapter that has no cancellation contract.
