...
Native development uses an Expo development client through the native scripts; native runtime acceptance is a separate validation gate and is not implied by web certification. Native runtime acceptance is currently unexecuted because the local iOS simulator destination was unavailable and Android tooling was unavailable.

The current static web export remains one approximately 4.5 MB bundle. Route-level splitting is an application-scale follow-up. See [docs/RELEASE_READINESS.md](docs/RELEASE_READINESS.md) for the current development, validation, and release gates.
