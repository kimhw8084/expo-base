...
const generator = read('packages/create-expo-base-app/bin/create-expo-base-app.mjs');
...
if (!bottomNavigation.includes('useExpoBaseDirection') || !bottomNavigation.includes('paddingStart: rt.insets.right') || !bottomNavigation.includes('paddingStart: rt.insets.left') || !bottomNavigation.includes('paddingEnd: rt.insets.right') || !bottomNavigation.includes('paddingEnd: rt.insets.left')) failures.push('Bottom navigation must map both physical safe-area edges to RTL-aware logical padding.');
if (!sidebarNavigation.includes('useExpoBaseDirection') || !sidebarNavigation.includes('paddingStart: rt.insets.right') || !sidebarNavigation.includes('paddingStart: rt.insets.left')) failures.push('Sidebar navigation must map the leading safe-area edge through RTL-aware logical padding.');
...
if (!referenceLinking.includes("appSchemes: ['expo-base', 'expo-base']")) failures.push('Reference linking must recognize its configured expo-base native scheme.');
