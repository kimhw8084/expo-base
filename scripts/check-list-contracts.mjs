...
const forbidden=[
  [/from\s+['"]@shopify\/flash-list['"]/,'direct alternate list virtualization; use @precision-calm/lists or add a reviewed adapter'],
  [/\bdisableVirtualization\s*=/,'virtualization must not be disabled in product screens'],
];
...
if(problems.length){console.error('List contract violations:\n'+problems.map((x)=>`- ${x}`).join('\n'));process.exit(1);}console.log('List contract check passed.');
