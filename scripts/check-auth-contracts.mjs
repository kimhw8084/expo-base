...
const provider=fs.readFileSync(path.join(root,'packages/runtime/src/ExpoBaseRuntimeProvider.tsx'),'utf8');
...
if(!provider.includes('<ExpoBaseAuthProvider'))problems.push('ExpoBaseRuntimeProvider must mount auth runtime for app services');
...
for(const marker of ['ProtectedRouterStack','useExpoBaseAuthAccess','useCaptureReturnIntent'])if(!refRoot.includes(marker))problems.push(`reference root missing ${marker}`);
