...
assert.ok(/const requestRevision = useRef\(0\)/.test(runtime));
assert.ok(/const activeAction = useRef/.test(runtime));
assert.ok(/const currentAction = activeAction\.current/.test(runtime));
assert.ok(/requestAtStart === requestRevision\.current/.test(runtime));
assert.ok(/refreshRequest = useRef/.test(runtime));
