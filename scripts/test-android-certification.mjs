...
const actionReadyMatcherStart = androidSource.indexOf('private Matcher<View> actionableTestIdMatcher');
const testIdIdentity = androidSource.slice(identityMatcherStart, actionReadyMatcherStart);
...
assert.ok(androidSource.includes('allOf(testIdMatcher(id), isDisplayed(), isEnabled())') && androidSource.includes('target.check(matches(actionableTestIdMatcher(id)))'), 'Regression: action readiness must be asserted after target discovery and scrolling.');
assert.ok(testIdReachabilityHelper.includes('onView(testIdMatcher(id))') && androidSource.includes('requestRectangleOnScreen') && androidSource.includes('instanceof ScrollView'), 'Regression: fully off-screen testIDs must use target-directed rectangle requests through the owning React Native ScrollView.');
