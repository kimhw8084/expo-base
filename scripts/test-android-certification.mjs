...
const accessibleReachabilityHelper = androidSource.slice(accessibleHelperStart);
...
assert.ok(androidSource.includes('clickTestId("overlay-action-menu-trigger"') && androidSource.includes('assertModalTestIdVisible("card-action-menu"'), 'Regression: delayed modal targets after a trigger must use bounded post-action readiness polling in the dialog root.');
...
assert.ok(accessibleReachabilityHelper.includes('requestAccessibleRectangleOnScreen(description, text)') && androidSource.includes('onView(targetMatcher)') && androidSource.includes('hasReactNativeScrollSurface') && !androidSource.includes('By.scrollable(true)') && !androidSource.includes('UiObject2'), 'Regression: off-screen id-less targets must use the actual React Native ScrollView through Espresso, not UI Automator scrollability.');
...
assert.ok(androidSource.includes('import static androidx.test.espresso.matcher.RootMatchers.isDialog;') && androidSource.includes('onViewInRoot(testIdMatcher(id), isDialog())') && androidSource.includes('target.inRoot(rootMatcher)') && androidSource.includes('import androidx.test.espresso.NoMatchingRootException;'), 'Regression: React Native Modal content must be resolved through an explicit Espresso dialog root.');
assert.ok(overlayScenario.includes('clickAccessibleTargetInModal("Edit card", "Edit card")') && androidSource.includes('scrollToAccessibleTarget(description, text, isDialog())'), 'Regression: id-less ActionMenu Edit interaction must remain modal-root scoped.');
assert.ok((overlayScenario.match(/device\.pressBack\(\);/g) ?? []).length === 2 && overlayScenario.includes('assertModalTestIdAbsent("overlay-dialog-review-action"') && overlayScenario.includes('assertModalTestIdAbsent("bottom-sheet-panel"') && androidSource.includes('catch (NoMatchingRootException dismissedRoot)'), 'Regression: Dialog and BottomSheet system Back dismissal must remain covered when their modal roots disappear.');
assert.ok(androidSource.includes('assertModalTestIdVisible("card-action-menu"') && androidSource.includes('assertModalTestIdVisible("bottom-sheet-panel"') && androidSource.includes('assertModalTestIdAbsent("overlay-dialog-review-action"') && androidSource.includes('assertModalTestIdAbsent("bottom-sheet-panel"') && androidSource.includes('doesNotExist()') && !androidSource.includes('Until.gone'), 'Overlay lifecycle and dismissal must fail closed on modal testID presence/disappearance.');
assert.ok(androidSource.includes('private ViewInteraction scrollToTestId(String id, String description)') && androidSource.includes('visibleTestIdMatcher(id), null') && androidSource.includes('private void clickHomeRoute') && androidSource.includes('clickTestId("home-route-" + route, label)'), 'Regression: ordinary activity-root route targets must not accidentally use the modal root.');
