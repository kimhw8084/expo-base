...
assert.ok(androidSource.includes('clickTestId("overlay-action-menu-trigger"') && androidSource.includes('assertModalAccessibilityVisible("card-action-menu", "Card actions"'), 'Regression: delayed modal targets after a trigger must use bounded post-action accessibility polling.');
...
assert.ok(!androidSource.includes('RootMatchers.isDialog') && !androidSource.includes('isDialog()') && !androidSource.includes('NoMatchingRootException') && !androidSource.includes('Matcher<Root>') && !androidSource.includes('import androidx.test.espresso.Root;') && modalAccessibilityHelper.includes('UiObject2') && modalAccessibilityHelper.includes('By.descContains') && modalAccessibilityHelper.includes('By.textContains') && modalAccessibilityHelper.includes('device.findObjects') && modalAccessibilityHelper.includes('isVisible()') && modalAccessibilityHelper.includes('isEnabled()'), 'Regression: React Native Modal content must use active-window UI Automator accessibility selectors.');
assert.ok(overlayScenario.includes('assertModalAccessibilityVisible("card-action-menu", "Card actions"') && overlayScenario.includes('clickModalAccessibilityTarget("Edit card", null, "Edit card")') && overlayScenario.includes('assertModalAccessibilityAbsent("card-action-menu", "Card actions"') && overlayScenario.includes('assertTestIdVisible("overlay-action-menu-trigger"') && modalAccessibilityHelper.includes('requireUniqueModalAccessibilityTarget'), 'Regression: ActionMenu Edit interaction must remain modal-accessibility scoped.');
assert.ok((overlayScenario.match(/device\.pressBack\(\);/g) ?? []).length === 2 && overlayScenario.includes('assertModalAccessibilityAbsent("overlay-dialog-review-action"') && overlayScenario.includes('assertModalAccessibilityAbsent("bottom-sheet-panel"') && overlayScenario.includes('assertTestIdVisible("overlay-dialog-trigger"') && overlayScenario.includes('assertTestIdVisible("overlay-bottom-sheet-trigger"'), 'Regression: Dialog and BottomSheet system Back dismissal must remain covered through modal accessibility.');
assert.ok(modalAccessibilityHelper.includes('matches.size() > 1') && modalAccessibilityHelper.includes('WAIT_MS / POLL_MS') && modalAccessibilityHelper.includes('Missing Android modal accessibility target') && androidSource.includes('assertModalAccessibilityAbsent("overlay-dialog-review-action"') && androidSource.includes('assertModalAccessibilityAbsent("bottom-sheet-panel"') && !androidSource.includes('Until.gone'), 'Overlay lifecycle must fail closed on modal ambiguity and semantic disappearance.');
