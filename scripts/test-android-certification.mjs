...
assert.ok(absentTargetBranch.includes('device.waitForIdle(POLL_MS);') && absentTargetBranch.includes('continue;') && !absentTargetBranch.includes('noProgressAttempts') && androidSource.includes('MAX_SCROLL_ATTEMPTS') && androidSource.includes('device.pressBack();\n    assertTestIdVisible("adapter-form-sections"'), 'Regression: delayed route targets after navigation or system Back must receive the full bounded absence wait.');
assert.ok(androidSource.includes('clickTestId("overlay-action-menu-trigger"') && androidSource.includes('assertTestIdVisible("card-action-menu"'), 'Regression: delayed overlay targets after a trigger must use bounded post-action readiness polling.');
