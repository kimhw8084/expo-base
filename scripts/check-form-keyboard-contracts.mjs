...
if (!adapter.includes('createExpoBaseFormKeyboardFlow')) failures.push('RHF adapter must own Next/Done keyboard flow.');
...
if (!reference.includes('createExpoBaseFormKeyboardFlow(form)')) failures.push('Reference form must prove the shared keyboard-flow adapter.');
if (!reference.includes('createExpoBaseFormSubmit(form')) failures.push('Reference form must use the shared interaction-stable submit owner.');
...
if (!reference.includes('useExpoBaseFormLifecycle(form, demoFormFields)')) failures.push('Reference form must prove post-commit deterministic invalid-field focus through the lifecycle owner.');
...
if (!adapter.includes('useExpoBaseFormErrorFocus') || !adapter.includes('submitCount') || !adapter.includes('handledSubmitCount') || !adapter.includes('form.setFocus(firstInvalid)')) failures.push('Shared form adapter must focus the first invalid field directly from the committed invalid-state effect.');
