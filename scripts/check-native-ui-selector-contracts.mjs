...
const checks = [
  ...
  ['overlay accessibility ambiguity and absence fail closed', modalAccessibilityHelper.includes('matches.size() > 1') && modalAccessibilityHelper.includes('WAIT_MS / POLL_MS') && modalAccessibilityHelper.includes('Missing Android modal accessibility target') && android.includes('assertModalAccessibilityAbsent("overlay-dialog-review-action"') && android.includes('assertModalAccessibilityAbsent("bottom-sheet-panel"') && !android.includes('Until.gone')],
