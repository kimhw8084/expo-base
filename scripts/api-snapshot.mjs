...
const snapshotFile=path.join(root,'expo-base.api.json');
const packages=['accessibility','components','data-display','feedback','forms','i18n','icons','layouts','lists','media-presentation','motion','navigation','overlays','patterns','primitives','visualization'];
const snapshot={schemaVersion:1,facade:'@expo-base/ui',packages:{}};
...
if(!fs.existsSync(snapshotFile)){console.error('expo-base.api.json missing; run npm run api:snapshot:write for an explicitly approved baseline.');process.exit(2);}
