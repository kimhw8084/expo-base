...
console.log(`Created Expo Base app at ${path.relative(root, destination) || destination}`);
...
function printUsage() { console.log('create-expo-base-app --name "App Name" --slug app-name [--accent blue|violet|green|orange] [--capabilities secure-storage,preferences,runtime-signals,sharing,media,local-auth,notifications,updates,device,haptics,observability] [--directory path] [--link-host app.example.com] [--force]'); }
