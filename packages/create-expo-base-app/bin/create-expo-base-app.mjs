...
function buildWorkspaceFiles(config, versions) {
  ...
  return {
    ...
    '.expo-base/task-effects.json': JSON.stringify({ schemaVersion: 1, actions: [] }, null, 2) + '\n',
...
function buildStandaloneFiles(config, versions, sourceRoot) {
  ...
  const rootPackage = {
    ...
    scripts: {
      ...
      'check:task-effects': 'node scripts/check-task-effects.mjs --path .',
      ...
      verify: 'npm run typecheck && npm run check:golden-patterns && npm run check:golden-architecture && npm run check:task-effects',
  ...
  files['scripts/check-task-effects.mjs'] = fs.readFileSync(path.join(sourceRoot, 'scripts', 'check-task-effects.mjs'), 'utf8');
