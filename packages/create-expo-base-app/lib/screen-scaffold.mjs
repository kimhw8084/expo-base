...
export function scaffoldScreen({ root, app, name, patternId, access = 'protected', capabilities = [] }) {
  ...
  const manifestPath = path.join(target, 'expo-base.routes.json');
  ...
  if (!routeManifest || routeManifest.schemaVersion !== 1 || !Array.isArray(routeManifest.authenticated) || !Array.isArray(routeManifest.public)) {
    throw new Error('This app has no valid expo-base.routes.json. Regenerate the app or add the canonical route manifest before scaffolding.');
  }
  const capabilityManifest = readJson(path.join(target, 'expo-base.capabilities.json'));
  ...
  for (const route of routes) if (existingRoutes.has(route)) throw new Error(`Route collision: "${route}" is already registered in expo-base.routes.json.`);
...
function transactionalWrite(target, files) {
  ...
  try {
    for (const [relative, content] of files) {
      ...
      const temporary = `${destination}.expo-base-scaffold-${process.pid}`;
