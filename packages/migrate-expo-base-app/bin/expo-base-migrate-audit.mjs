...
const compatibilityPath = path.join(workspaceRoot, 'expo-base.compatibility.json');
const compatibility = fs.existsSync(compatibilityPath) ? JSON.parse(fs.readFileSync(compatibilityPath, 'utf8')) : {};

const rules = [
  rule('STYLE-001','high','foundation','Inline style object',/style\s*=\s*\{\s*\{/g,'Move geometry/appearance into Expo Base primitives and semantic variants.'),
  rule('STYLE-002','medium','foundation','Feature StyleSheet',/\bStyleSheet\.create\s*\(/g,'Replace feature-owned styling with @expo-base/ui composition.'),
  ...
  rule('ICON-001','medium','behavior','Direct icon/SVG implementation',/from\s+['\"](?:lucide-react-native|react-native-svg|@expo\/vector-icons)['\"]/g,'Use the semantic @expo-base/ui Icon facade.'),
  rule('MOTION-001','medium','behavior','Direct animation/haptics implementation',/from\s+['\"](?:react-native-reanimated|expo-haptics)['\"]/g,'Use Expo Base motion/haptic semantics so reduced-motion behavior stays consistent.'),
  ...
  rule('LINK-001','high','behavior','Direct external/deep-link implementation',/from\s+['"](?:expo-linking|expo-web-browser)['"]|import\s*\{[^}]*\bLinking\b[^}]*\}\s*from\s*['"]react-native['"]|\bLinking\.(?:openURL|canOpenURL)\s*\(|\bwindow\.open\s*\(|\b(?:window\.)?location\.href\s*=|<a\b[^>]*\bhref\s*=|\bhref\s*=\s*['"]https?:\/\//g,'Route outgoing URLs through the Expo Base linking runtime so schemes/hosts are validated centrally.'),
  rule('AUTH-001','high','behavior','Feature owns authentication adapter/session routing',/\bservices\.auth\.(?:getSession|signIn|signOut|subscribe)\s*\(|\b(?:router|navigation)\.(?:replace|push|navigate)\s*\([^)]*['"](?:\/)?sign-in['"]/g,'Use useExpoBaseAuth plus ProtectedRouterStack so session resolution, history invalidation, and protected-route access stay centralized.'),
  rule('AUTHZ-001','high','behavior','Feature owns authorization/role checks',/\bservices\.authorization\.(?:getCapabilities|subscribe)\s*\(|\b(?:user|session\.user)\.role\s*===|\broles?\.includes\s*\(/g,'Use useExpoBaseAuthorization/useExpoBaseAuthorizationRequirement so capability loading/error states fail closed and route/UI gating stays centralized.'),
  rule('BACKEND-001','high','integration','Backend client imported in feature UI',/from\s+['\"](?:@supabase\/[^'\"]+|firebase(?:\/[^'\"]*)?)['\"]/g,'Move backend access behind @expo-base/adapters/service composition.'),
  rule('API-001','medium','foundation','Direct Expo Base implementation import',/from\s+['\"]@expo-base\/(?:accessibility|components|data-display|feedback|forms|icons|layouts|lists|motion|navigation|overlays|patterns|primitives|visualization)['\"]/g,'Feature screens should import visual APIs from @expo-base/ui.'),
];
...
for (const file of sourceFiles(root)) {
  ...
  if (!isRouteLayout) {
    for (const match of text.matchAll(/from\s+['\"]expo-router['\"]/g)) findings.push({ id:'NAV-001', severity:'medium', wave:'behavior', title:'Feature route coupled directly to Expo Router', file:relative, line:lineAt(text,match.index ?? 0), sample:match[0], recommendation:'Use @expo-base/navigation-router so route-library coupling stays at the adapter boundary.' });
  }
}
...
function printHuman(result){
  console.log('Expo Base migration audit'); console.log(`Path: ${result.auditedPath}`); console.log(`Findings: ${result.summary.total} (${result.summary.bySeverity.high} high, ${result.summary.bySeverity.medium} medium, ${result.summary.bySeverity.low} low)`); console.log(`Files affected: ${result.summary.filesAffected}`);
...
function usage(){ console.log('expo-base-migrate-audit [--path app-or-repo] [--json] [--fail-on low|medium|high]'); }
function findWorkspaceRoot(start){ let current=path.resolve(start); while(true){if(fs.existsSync(path.join(current,'expo-base.compatibility.json')))return current;const parent=path.dirname(current);if(parent===current)return start;current=parent;} }
